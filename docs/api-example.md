...
## История запросов и журнал проверок (GET /v2/requests)

Для получения списка всех ранее созданных проверок (заявок), их статусов и параметров используйте эндпоинт **`GET https://api.newdb.net/v2/requests`** (или алиас `GET /v2/history`).

Поддерживается пагинация (`page`, `limit`), фильтрация и режим совместимости с форматом «ЗаЧестныйБизнес» (`/flcheck/data/list` или параметр `format=zb`).
...
Подробное описание параметров, структуры ответа и примеры интеграции приведены в отдельном разделе [История запросов (Журнал)](requests.md).
